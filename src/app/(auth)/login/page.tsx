...
const URL_ERRORS: Record<string, string> = {
  oauth_failed: "Google sign-in is not available. Please use email and password.",
  auth_callback_failed: "The sign-in link has expired or is invalid. Please try again.",
};

function UrlErrorBanner({ formError }: { formError?: string }) {
  const searchParams = useSearchParams();
  const urlError = URL_ERRORS[searchParams.get("error") ?? ""] ?? null;
  const message = formError ?? urlError;
...
function LoginForm() {
  ...
  return (
    <div style={{ width: "100%", maxWidth: 400 }}>
      ...
      {/* Error banner — reads from URL params (needs Suspense) and form state */}
      <Suspense fallback={null}>
        <UrlErrorBanner formError={state?.error} />
      </Suspense>
